# 创建bbox_regression数据集（用于边界框回归器训练）
...
if __name__ == '__main__':
    """
    从voc_cat/train目录中提取标注边界框坐标
    从finetune_cat/train目录中提取训练集正样本坐标（IoU>=0.5），进一步提取IoU>0.6的边界框
    数据集保存在bbox_cat目录下
    """
    voc_cat_train_dir = '../data/voc_cat/train'
    # ground truth
    gt_annotation_dir = os.path.join(voc_cat_train_dir, 'Annotations')
    jpeg_dir = os.path.join(voc_cat_train_dir, 'JPEGImages')

    classifier_cat_train_dir = '../data/finetune_cat/train'
    # positive
    positive_annotation_dir = os.path.join(classifier_cat_train_dir, 'Annotations')
    ...
    samples = util.parse_cat_csv(voc_cat_train_dir)
    res_samples = list()
